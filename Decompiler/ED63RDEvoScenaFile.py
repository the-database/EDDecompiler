from ED63RDEvoScenarioScript import *
from Instruction.ScenaOpTableED63RDEvo import *
...
def updateScnInfoOffset(current):
    ...
    #     print('maxed', scena.HeaderEndOffset)
    # except AttributeError:
    #     print('not maxed')
    #     scena.HeaderEndOffset = scena.fs.tell()

    # for i in range(2):
    #     if len(scena.ScnInfo[i]) == 0:
    #         scena.HeaderEndOffset += 1
    #         print('hacked', hex(scena.HeaderEndOffset))
    
    for i in range(current + 1, SCN_INFO_MAXIMUM):
        scena.ScnInfoOffset[i] = scena.HeaderEndOffset
        print(i, hex(scena.ScnInfoOffset[i]))
        # for i in range(2):
        # evo hack: shift index for ScenarioChipInfo if empty
        # if len(scena.ScnInfo[i]) == 0:
            # scena.ScnInfoOffset[i] = scena.ScnInfoOffset[i] + (i + 1) * -1
...
def SaveToFile():
    ...
    print('tell', hex(fs.tell()))
    print(len(scena.binary()))

    # fs.seek(0x64 + 0x44)
    # evo hack: add FF
    # if len(scena.ScnInfo[0]) == 0:
        # print('hack!')
        # scena.fs.write(b'\xFF')
    # if len(scena.ScnInfo[1]) == 0:
        # print('hack!!')
        # scena.fs.write(b'\xFF')
