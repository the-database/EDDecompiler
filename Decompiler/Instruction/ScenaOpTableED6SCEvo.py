...
from Base.ED6SCEvoBase import *
from GameData.ItemNameMapSCEvo import *
...
class ED6SCScenaInstructionTableEntry(InstructionTableEntry):
    ...
    def GetOperand(self, opr, fs):
        ...
        def readstr():
            ...
            while True:
                ...
                elif (buf >= b'\x80' and buf <= b'\x9f') or buf >= b'\xe0':
...
def inst(op, operand = NO_OPERAND, flags = 0, handler = None):
    return ED6SCScenaInstructionTableEntry(op, InstructionNames[op], operand, flags, handler)
...
# ExpressionOperantions[0xD5] = 'EXPR_D5'
...
class ScpExpression:
    ...
    def __repr__(self):
        try:
            return '%s %s' % (ExpressionOperantions[self.Operation], str(self.Operand))
        except KeyError:
            return '%s %s' % (self.Operation, str(self.Operand))

    def __str__(self):

        # print(hex(self.Operation), self.Operand)
        ...
        if self.Operation == EXPR_TEST_SCENA_FLAGS:
            ...
            print(self.Operation, self.Operand)
...
def scp_2a(data):
    ...
    if data.Reason == HANDLER_REASON_DISASM:
        ...
        data.Instruction.OperandFormat = 'W' * min(0x20, len(data.Arguments))
...
def scp_ce(data):
    ...
    if data.Reason == HANDLER_REASON_DISASM:
        ...
        opr1, opr2, opr3, opr4, opr5, opr6, opr7 = data.TableEntry.GetAllOperand('BBBBBBB', fs)

        ins.Operand.append(opr1)
        ins.Operand.append(opr2)
        ins.Operand.append(opr3)
        ins.Operand.append(opr4)
        ins.Operand.append(opr5)
        ins.Operand.append(opr6)
        ins.Operand.append(opr7)

        operand = getopr(opr7)

        ins.Operand += data.TableEntry.GetAllOperand(operand, fs)

        ins.OperandFormat = 'BBBBBBB' + operand

        return ins

    elif data.Reason == HANDLER_REASON_ASSEMBLE:

        opr7 = data.Arguments[6]
        operand = getopr(opr7)

        data.Instruction.OperandFormat = 'BBBBBBB' + operand
...
def scp_41(data):
    def getopr(itemid):
        return 'B' if itemid in [0x258, 0x259, 0x25A, 0x25B, 0x25C, 0x25D, 0x25E, 0x25F, 0x260, 0x261, 0x262, 0x263, 0x264, 0x265, 0x266, 0x267, 0x268, 0x269, 0x26A, 0x26B, 0x26C, 0x26D, 0x26E, 0x26F, 0x270, 0x271, 0x272, 0x273, 0x274, 0x275, 0x276, 0x27D, 0x27E, 0x27F, 0x280, 0x281, 0x282, 0x283, 0x284, 0x285, 0x286, 0x287, 0x288, 0x28A, 0x28B, 0x28E, 0x28F, 0x291, 0x2C1, 0x2C2, 0x2C3, 0x2C6, 0x2C7, 0x2C8, 0x2C9, 0x2CA, 0x2D0, 0x2D1, 0x2D2, 0x2D3, 0x2D4, 0x2D7, 0x315, 0x316, 0x317, 0x318, 0x319, 0x31A, 0x31B, 0x31C, 0x31D, 0x31E, 0x31F] else ''
...
def scp_ed(data):
    print('scp_ed reason', data.Reason)
    if data.Reason == HANDLER_REASON_FORMAT:
        print('ed?')
        return FormatFuncString(data, data.Instruction.OperandFormat, -1)
...
[
    ...
    inst(Call,                      'CCC'),          # Call(scp index, func index)
    ...
    inst(OP_55),
    inst(OP_56,                     'B'),
    inst(OP_57,                     'BBBBBBS'), #TODO wrong. max length: 6
    ...
    inst(OP_BC,                     'B'),
    inst(OP_BD),
    inst(OP_BE,                     'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB'),
    inst(OP_CB,                     'B'),
    inst(SetChrChipPat,             'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB'),                         # SetChrChipPat(chr_id, func_id, param)
    inst(LoadChrChipPat),
    inst(OP_D0,                   'BBBBBBBB'),
    inst(OP_D1,                      'BBBBBBBBBBBBBBBBBB'),
    inst(OP_DE,                      'S'),

    inst(OP_BF,                     'BBBBBB'),
    inst(OP_C1,                        'BBBBBB'),
    inst(OP_C4,                       'BL'),
    inst(OP_C5,                       'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBS'),
    # inst(OP_C9,                     'BBBBWWWWWWWWW'),
    inst(OP_C9,                     NO_OPERAND,             0,                          scp_c9),
    inst(OP_CA,                     'BBL'),
    inst(OP_CC,                     NO_OPERAND,             0,                          scp_cc),
    inst(OP_CD,                     'BB'),
    inst(OP_CE,                     NO_OPERAND,             0,                          scp_ce), # min 6. not 4
    # inst(OP_CE,                     'BBBBBBBB'),
    inst(OP_CF,                     'WBS'),
    inst(OP_D2,                     'BBBBBBBBB'), # TODO
    inst(OP_D3,                     'B'),
    inst(OP_D5,                     'B'),
    inst(OP_D6,                     'B'),
    inst(OP_D7,                     'BBBBBBB'),
    inst(OP_D8,                     'BBB'),
    # inst(OP_D9,                     'BB'),
    inst(OP_E0,                     'BBBBBBBBBBBBB'),
    inst(OP_E1,                     'BBBBBBBBBBBBB'),
    # inst(OP_E2,                     'B'),
    ...
    inst(OP_EB,                    'BB'),
    inst(OP_EC,                    'W'),
    inst(OP_ED,                    'S',                    0,                           scp_ed),
    inst(OP_EE,                     'BB'),
    inst(OP_C6,                     'BBBBBBBBBBBBBB'),
    inst(OP_C7,                     'BBB'),
    inst(OP_C8,                     'BBBBSBBB'),
    inst(OP_DA),
    inst(OP_DB),
